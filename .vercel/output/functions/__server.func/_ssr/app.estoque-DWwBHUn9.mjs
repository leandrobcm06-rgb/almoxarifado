...
//#region node_modules/.nitro/vite/services/ssr/assets/app.estoque-DWwBHUn9.js
...
function Page() {
	...
	const confirm = useMutation({
		mutationFn: async () => {
			...
			let page = 0;
			while (true) {
				const { data, error } = await supabase.from("products").select("id, codigo").range(page * 1e3, (page + 1) * 1e3 - 1);
		...
		onSuccess: () => {
			toast.success("Estoque confirmado com sucesso!");
			setPending([]);
			qc.invalidateQueries({ queryKey: ["snapshots"] });
		},
		onError: (e) => {
			console.error(e);
			toast.error(e?.message || "Erro desconhecido ao confirmar");
		}
	});
