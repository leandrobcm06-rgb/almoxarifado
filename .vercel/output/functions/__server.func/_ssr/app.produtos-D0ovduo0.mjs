...
//#region node_modules/.nitro/vite/services/ssr/assets/app.produtos-D0ovduo0.js
...
function Page() {
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			let page = 0;
			while (true) {
				const { data: rows } = await supabase.from("products").select("*").order("codigo").range(page * 1e3, (page + 1) * 1e3 - 1);
	...
	const { data: totals } = useQuery({
		...
		queryFn: async () => {
			...
			if (!snaps || snaps.length === 0) return /* @__PURE__ */ new Map();
